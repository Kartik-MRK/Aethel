...
**S1.1 — Silent data corruption.** `aethel/commands/commit.py:197-221` returns early when an
...
| **`DEMO_GUIDE.md` structure** | — | Well built; folds into `docs/demo.md`. |
| **`core/aggregator.py`** | — | Dead today, but it becomes the Hub's transparency log. Keep and test it. |
...
.aethel/
  ...
  index.db                         -> DERIVED CACHE ONLY, rebuilt by `aethel reindex`
```
...
1. **Commits are keyed by commit hash, never by patch hash.** Identical weights share a blob and
   ...
   representable form.
2. **SQLite is a pure cache.** Every read path must work from the object store alone; `aethel
   reindex` rebuilds the DB from objects. This makes the "portable — just copy the folder" claim
   true, which today it is not.
...
**Deleted:** `experiment_lora.py`, `aethel/core/model.py`, `demo_flow.ps1`,
...
`scripts/dev.{sh,ps1}`, `.github/workflows/ci.yml`, `docs/design/`.
...
- **Portability:** delete `index.db`, run `aethel reindex`, full history recovers from objects alone
